...
qtflow deploy <target>              # 打包 Qt 运行时 DLL/插件到可执行文件旁
```
...
| `deploy` | 用 windeployqt/macdeployqt 打包 Qt 运行时 |
| `plan` | 只渲染命令计划,不执行 |
...
在 Windows 上,当 `[msvc].enabled = true` 且未传 `--no-msvc-bootstrap` 时,qtflow 会在执行 configure/build/test/deploy 前自动初始化 MSVC 环境:解析 `VsDevCmd.bat`,再通过 `cmd.exe` 以 `call "<VsDevCmd.bat>" -arch=<arch>` 包裹命令执行。

`VsDevCmd.bat` 解析优先级:`--vsdevcmd` → `QTFLOW_VSDEVCMD_BAT` → `VSDEVCMD_BAT` → 配置 → `VSINSTALLDIR` → `vswhere` → 已知安装路径。

## Deploy

`qtflow deploy [target]` 会调用 Qt 官方部署工具,Windows 用 `windeployqt`,macOS 用 `macdeployqt`,把 Qt 运行时 DLL/插件打包到已构建的可执行文件旁。Linux 没有 Qt 官方部署工具,qtflow 会以退出码 3 给出明确提示;可考虑 `linuxdeployqt` 或手工打包。
...
- `--exe <path>`: 显式指定 exe 或 macOS `.app`,相对路径按项目根解析。
- `--release` / `--debug`: 指定部署 release/debug Qt 运行时;不传时按 profile/config name 推断,包含 `release` 就按 release,否则 debug。
- `--qmldir <dir>`: 传给部署工具的 QML 目录。
- `--dir <path>`: 输出目录;默认放在 exe 旁边。
- `--deploy-arg <arg>`: 透传额外参数,可重复。
- `--no-msvc-bootstrap`、`--vsdevcmd <path>`: 与 build/check 相同的 MSVC 控制。

未传 `--exe` 时,qtflow 会在当前 profile 的 `build_dir` 下优先找 `bin/<target>.exe`,再找 `<target>.exe`,再做浅层递归搜索;macOS 同时接受 `<target>.app`。`plan` / `--dry-run` 找不到 exe 时仍会渲染最可能路径并附 note;真实执行会以退出码 4 提示先 `qtflow build <target>` 或使用 `--exe`。
...
命令失败时,qtflow 会输出结构化诊断(文本 + `--json`)并给出修复建议,而不是堆原始日志。覆盖常见问题:MSVC 标准头缺失、`VsDevCmd` 未找到、构建目录缺失、preset 缺失、cmake/ctest 未找到、Qt 运行时 DLL 缺失、deploy 工具缺失、CTest 无匹配测试等。
