...
pub fn plan_actions(
    ...
    plan_actions_inner(root, opts, detected, fs, inputs, None)
}

pub fn plan_actions_for_project(
    root: &Path,
    opts: &InitOptions,
    detected: &DetectedAgents,
    fs: &impl InitFileSystem,
    inputs: &InitConfigInputs,
    project: &InitProjectInputs,
) -> Result<InitPlan, InitPlanError> {
    plan_actions_inner(root, opts, detected, fs, inputs, Some(project))
}

fn plan_actions_inner(
    root: &Path,
    opts: &InitOptions,
    detected: &DetectedAgents,
    fs: &impl InitFileSystem,
    inputs: &InitConfigInputs,
    project: Option<&InitProjectInputs>,
) -> Result<InitPlan, InitPlanError> {
    ...
    if !opts.no_config {
        let (action, config_warnings) = plan_config_action(root, opts, fs, inputs, project)?;
...
fn qmake_starter_config_toml(selection: QmakeInitConfigSelection) -> String {
    let mut output = String::new();
    output.push_str("# qtflow qmake starter config\n");
    output.push_str(
        "# Generated by `qtflow init` for a .pro project. Edit paths to match your project.\n\n",
    );
    output.push_str("build_system = \"qmake\"\n");
    output.push_str(&format!("default_profile = \"{DEFAULT_PROFILE_NAME}\"\n\n"));
    output.push_str("[qmake]\n");
    output.push_str(&format!(
        "# pro_file source: discovered .pro project\npro_file = \"{}\"\n",
        toml_escape(&path_to_slash(&selection.pro_file))
    ));
    output.push_str("# auto-detect spec, for example win32-msvc or linux-g++\nspec = \"\"\n");
    output.push_str(
        "# auto-detect make from spec, for example nmake, jom, mingw32-make, or make\nmake = \"\"\n",
    );
    output.push_str(&qmake_profile_toml(DEBUG_PROFILE_NAME, &selection.debug));
    output.push_str(&qmake_profile_toml(
...
fn qmake_profile_toml(name: &str, profile: &InitProfileSelection) -> String {
    let mut output = String::new();
    output.push_str(&format!("\n[profiles.{name}]\n"));
    if let Some(warning) = &profile.build_dir_warning {
        output.push_str(&format!("# {warning}\n"));
...
fn plan_config_action(
    ...
    let config = select_starter_config(root, opts, inputs, project)?;
    ...
    Ok((
        InitAction {
            ...
            source: Some(source),
            content: will_write.then(|| config.into_toml()),
            operation: if will_write && !opts.dry_run {
                WriteOperation::Write
            } else {
                WriteOperation::None
...
struct StarterConfig {
    source: String,
    warnings: Vec<String>,
    kind: StarterConfigKind,
}

#[derive(Debug, Clone, PartialEq, Eq)]
enum StarterConfigKind {
    Cmake(InitConfigSelection),
    Qmake(QmakeInitConfigSelection),
}

impl StarterConfig {
    fn into_toml(self) -> String {
        match self.kind {
            StarterConfigKind::Cmake(selection) => starter_config_toml(selection),
            StarterConfigKind::Qmake(selection) => qmake_starter_config_toml(selection),
        }
    }
}

fn select_starter_config(
    root: &Path,
    opts: &InitOptions,
    inputs: &InitConfigInputs,
    project: Option<&InitProjectInputs>,
) -> Result<StarterConfig, InitPlanError> {
    if let Some(project) = project.filter(|project| project.kind == ProjectKind::Qmake) {
        let selection = select_qmake_config(root, opts, project);
        return Ok(StarterConfig {
            source: selection.source.clone(),
            warnings: selection.warnings.clone(),
            kind: StarterConfigKind::Qmake(selection),
        });
    }
    ...
    Ok(StarterConfig {
        source: selection.source.clone(),
        warnings: selection.warnings.clone(),
        kind: StarterConfigKind::Cmake(selection),
    })
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct QmakeInitConfigSelection {
    source: String,
    pro_file: PathBuf,
    debug: InitProfileSelection,
    release: InitProfileSelection,
...
fn select_qmake_config(
    root: &Path,
    opts: &InitOptions,
    project: &InitProjectInputs,
) -> QmakeInitConfigSelection {
    let fallback = qmake_selection_from_defaults(root, project);
    let debug = opts
        .build_dir_debug
        .as_ref()
        .map(|path| qmake_override_profile(root, path))
    ...
    let override_used = opts.build_dir_debug.is_some() || opts.build_dir_release.is_some();

    QmakeInitConfigSelection {
        source: if override_used {
            "qmake override".to_string()
        } else {
            "qmake defaults".to_string()
        },
        pro_file: relative_to_root(root, &project.project_file),
        debug,
        release,
        warnings: Vec::new(),
    }
...
fn qmake_selection_from_defaults(
    root: &Path,
    project: &InitProjectInputs,
) -> QmakeInitConfigSelection {
    QmakeInitConfigSelection {
        source: "qmake defaults".to_string(),
        pro_file: relative_to_root(root, &project.project_file),
        debug: qmake_default_profile("build/debug", "debug"),
        release: qmake_default_profile("build/release", "release"),
        warnings: Vec::new(),
    }
}

fn qmake_default_profile(build_dir: &str, config_name: &str) -> InitProfileSelection {
    InitProfileSelection {
        build_dir: PathBuf::from(build_dir),
        generator: None,
        config_name: Some(config_name.to_string()),
        preset: None,
        comment: "qmake defaults - no CMake cache discovered".to_string(),
        build_dir_warning: None,
    }
}

fn qmake_override_profile(root: &Path, path: &Path) -> InitProfileSelection {
    InitProfileSelection {
        build_dir: relative_to_root(root, path),
        generator: None,
        config_name: None,
        preset: None,
        comment: "qmake override".to_string(),
        build_dir_warning: None,
    }
}
...
mod tests {
    ...
    fn qmake_project() -> InitProjectInputs {
        InitProjectInputs {
            kind: ProjectKind::Qmake,
            project_file: PathBuf::from("/repo/app.pro"),
        }
    }
    ...
    #[test]
    fn qmake_config_inference_generates_qmake_starter_config() {
        let fs = MemoryInitFileSystem::default();
        let plan = plan_actions_for_project(
            &root(),
            &opts(),
            &DetectedAgents::default(),
            &fs,
        ...
        assert_eq!(plan.actions[0].source.as_deref(), Some("qmake defaults"));
        assert!(toml.contains(r#"build_system = "qmake""#));
        assert!(toml.contains("[qmake]"));
        assert!(toml.contains(r#"pro_file = "app.pro""#));
        assert!(toml.contains(r#"spec = """#));
        assert!(toml.contains(r#"make = """#));
        assert!(toml.contains(r#"build_dir = "build/debug""#));
        assert!(toml.contains(r#"build_dir = "build/release""#));
        assert!(!toml.contains("preset ="));
        assert!(!toml.contains("ctest_args"));
    }

    #[test]
    fn qmake_config_inference_honors_build_dir_overrides() {
        let fs = MemoryInitFileSystem::default();
        let options = InitOptions {
        ...
        assert_eq!(plan.actions[0].source.as_deref(), Some("qmake override"));
        assert!(toml.contains("# build_dir source: qmake override"));
